...
class PVS {
	...
	withTransposition: boolean = false;
	withPathCash: boolean = true;
	withAdjacentWalls: boolean = true;
	withCentrality: boolean = true;

	rootNode: TreeNode = { depth: 5, move: undefined, score: 0, children: [] };
	...
	private pvs(depth: number, a: number, b: number, color: number, parentNode: TreeNode): [number, Move | undefined] {
		...
		for (let i = 0; i < possibleMoves.length; i++) {
			...
			if (i === 0) {
				score = -this.pvs(depth - 1, -b, -a, -color, childNode)[0];
			} else {
				score = -this.pvs(depth - 1, -a - 1, -a, -color, childNode)[0];
				if (score > a && score < b) {
					score = -this.pvs(depth - 1, -b, -a, -color, childNode)[0];
				}
			}
	...
	private evaluatePosition(color: number): number {
		...
		if (this.withPathCash) {
			...
			if (!this.shortestPath1 || isLastMovePlayerMove || !this.isPathWalkable(this.shortestPath1)) {
			...
			if (!this.shortestPath2 || isLastMovePlayerMove || !this.isPathWalkable(this.shortestPath2)) {
	...
	private isPathWalkable(path: Node[]): boolean {
		...
		for (let i = 0; i < path.length - 1; i++) {
			...
			if (
				(deltaX === -1 && !this.game.checkWalkableLeft(currentNode.x, currentNode.y)) ||
